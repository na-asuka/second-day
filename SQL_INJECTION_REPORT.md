...
**问题：** 旧版 `index()` 只查 `USERS` 内存字典，通过 `/register` 注册的新用户登录后字典查不到，页面显示"请先登录"。

**修复：** `index()` 改为从数据库 `SELECT` 查询用户信息，漏洞版用 f-string 保持可注入，安全版用参数化查询。

### ② 安全版密码全流程哈希

**问题：** 旧版安全版注册存明文密码，且登录验证只认字典里的 bcrypt 哈希，注册用户永远无法登录。

**修复：** 注册时 `hash_pw(p)` 后入库，登录验证查数据库 `verify_pw()`。

### ③ 布尔盲注指示灯

漏洞版首页新增 **✅ 条件为真** / **❌ 条件为假** 状态指示灯 + 行数统计，让盲注结果一目了然。

### ④ 安全版参数化SQL展示

安全版搜索页新增 **预编译SQL** 和 **绑定参数** 显示，直观对比 `?` 占位符 vs f-string 拼接的区别。
...
- **系统异常：** `logger.error()` 记录详情，前端返回通用"查询异常，请稍后再试"
- **用户输入异常：** 返回具体校验错误信息
- **不向用户暴露 SQL 错误细节**
