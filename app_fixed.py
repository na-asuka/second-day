"""
====================================================================
  安全修复版 — 参数化查询 (?) 防止 SQL 注入
  端口: 5001
  修复方式: 全部 SQL 使用 ? 占位符
  功能: 登录 / 注册 / 搜索 / 信息展示（与漏洞版功能一致）
====================================================================
"""
...
def index():
    ...
    return render_template("index_safe.html", username=uname, user=ui)
...
def search():
    ...
    return render_template("index_safe.html", username=uname, user=ui, search_results=results, keyword=keyword)
...
def register():
    if request.method == "POST":
        ...
        try:
            conn=sqlite3.connect(DB_PATH);c=conn.cursor();c.execute(sql, (u, p, e, ph));conn.commit();conn.close()
            return render_template("login.html",error="注册成功，请登录")
        except: return render_template("register_safe.html",error="注册失败")
